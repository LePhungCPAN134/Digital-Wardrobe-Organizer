...
async function updateExistingClothingItem(clothingID, newClothing) {
  ...
  if (!updated) throw new Error(`Clothing with ${clothingID} doesn't exist`);
  return updated;
}
 ...
 */
async function deleteClothingItem(clothingID) {
  if (!clothingID) throw new Error(`Cannot delete clothing item`);
  const deleted = await ClothingModel.findByIdAndDelete(clothingID).lean();
  if (!deleted) throw new Error(`Clothing with ${clothingID} doesn't exist`);
  return deleted;
}
